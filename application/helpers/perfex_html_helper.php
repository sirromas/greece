...
function render_yes_no_option($option_value, $label, $tooltip = '', $replace_yes_text = '', $replace_no_text = '', $replace_1 = '',$replace_0 = '')
{
    ...
    <div class="form-group">
        <label for="<?php echo $option_value; ?>" class="control-label clearfix">
            <?php echo ($tooltip != '' ? '<i class="fa fa-question-circle" data-toggle="tooltip" data-title="'. _l($tooltip,'',false) .'"></i> ': '') . _l($label); ?>
        </label>
...
function app_external_form_header($form){
  ...
  <?php do_action('app_external_form_head'); ?>
 <?php
}
...
function AdminTicketsTableStructure($name = '', $bulk_action = false)
{
   ob_start();
?>
<table class="table dt-table-loading <?php echo $name == '' ? 'tickets-table' : $name;?> table-tickets">
<thead>
<tr>
<?php if ($bulk_action == true) { ?>
<th>
...
<th<?php if(get_option('services') == 0){echo ' class="not_visible"'; }?>><?php echo _l('ticket_dt_service');?></th>
...
<th class="ticket_created_column"><?php echo _l('ticket_date_created');?></th>
<?php
...
if($CI->session->has_userdata('system-popup')) { ?>
        <script>
            $(function(){
                var popupData = {};
                popupData.message = <?php echo json_encode(app_happy_text($CI->session->userdata('system-popup'))); ?>;
